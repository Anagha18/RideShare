...
def addride():
	...
	w=requests.post("http://127.0.0.1:8000/api/v1/db/read", json=d)
	print(w.text)
	...
	if(e):
		d['flag'] = 3
		#return jsonify(d)
		r=requests.post("http://127.0.0.1:8000/api/v1/db/write", json=d)
		return str(r.text)
		return jsonify({}), 201
	else :
		return jsonify({}), 400
...
def viewridesource():
	...
	# print(type(dest))
	if(s and desti ):
		rr1=requests.post("http://127.0.0.1:8000/api/v1/db/read", json=d)
...
def viewridedetails(ride_id):
	...
	d['rideId'] = ride_id
	exus1=requests.post("http://127.0.0.1:8000/api/v1/db/read", json=d)
	...
	di['rideId'] = ride_id
	ride1=requests.post("http://127.0.0.1:8000/api/v1/db/read", json=di)
	...
	dit['rideId'] = ride_id
	ursr1=requests.post("http://127.0.0.1:8000/api/v1/db/read", json=dit)
	ursr =ast.literal_eval(ursr1.text)
	
	dito = dict()
	dito['flag'] = 6 #copy her 8 th one
	dito['rideId'] = ride_id
	r1=requests.post("http://127.0.0.1:8000/api/v1/db/read", json=dito)
	r =r1.json()
	r2=r['val']
...
def readFromDB():
    ...
    if flag == 1:  #check if username is present
        ...
        l=requests.get("http://172.17.0.1:8080/api/v1/users")
        print("\n\n\n\n\n",type(l),"\n\n\n\n\n")
        l=l.json()
        if(username in l):
        #if(u):
